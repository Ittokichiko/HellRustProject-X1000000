...
{
    [Info("Rustcord", "Kirollos & OuTSMoKE", "3.4.2")]
    ...
    internal class Rustcord : RustPlugin, IDiscordPlugin
    {
        ...
        protected override void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                //Changed by Terceran 20240619
                //{ "RUST_CustomLog", ":desktop: [Custom] {logtext}"},
                { "RUST_CustomLog", " {logtext}"},
        ...
        private void OnDeathNotice(Dictionary<string, object> data, string message)
        {
            ...
            if (_DeathNotes != null)
            {
                ...
                if (_DeathNotes.Call("IsDeathPVP", victimType, killerType) is bool isPVP && isPVP)
                {
                    for (int i = 0; i < _channelCount; i++)
                    {
                ...
                else if (_DeathNotes.Call("IsDeathAnimal", victimType, killerType) is bool isAnimal && isAnimal)
                {
                    for (int i = 0; i < _channelCount; i++)
                    {
                ...
                else if (_DeathNotes.Call("IsDeathVehicle", victimType, killerType) is bool isVechicle && isVechicle)
                {
                ...
                else if (_DeathNotes.Call("IsDeathNPC", victimType, killerType) is bool isNPC && isNPC)
                {
                    message = (string)_DeathNotes.Call("StripRichText", message);
